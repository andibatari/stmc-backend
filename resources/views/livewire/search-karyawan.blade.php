<div>
    <div class="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
        <div class="flex flex-col md:flex-row items-center gap-4 w-full">
            @if ($activeTab === 'ptst')
                <input type="text" wire:model.live.debounce.500ms="searchSap" placeholder="Cari SAP Karyawan" class="w-full md:w-auto p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                <input type="text" wire:model.live.debounce.500ms="searchNama" placeholder="Cari Nama Karyawan" class="w-full md:w-auto p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                <input type="text" wire:model.live.debounce.500ms="searchUnitKerja" placeholder="Cari Unit Kerja" class="w-full md:w-auto p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
            @else
                <input type="text" wire:model.live.debounce.500ms="searchNamaPasien" placeholder="Cari Nama Pasien" class="w-full md:w-auto p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
                <input type="text" wire:model.live.debounce.500ms="searchNik" placeholder="Cari Nik" class="w-full md:w-auto p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500">
            @endif
        </div>
    ...
    <div class="flex justify-end mb-6 space-x-2"> 
        ...
        @if ($activeTab === 'non-ptst')
            ...
            <a href="{{ route('pasien.add.nonkaryawan') }}" class="flex items-center justify-center bg-red-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-red-700 transition-colors duration-200"> 
                <svg class="h-6 w-6 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg> 
                Tambah Pasien 
            </a> 
            <a href="{{ route('peserta.mcu.download') }}" class="w-full md:w-auto flex items-center justify-center bg-red-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-red-700 transition-colors duration-200">
                <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                <span>Download</span>
            </a>
        @endif
    ...
    <div class="flex border-b border-gray-200 mb-6 space-x-4">
        <button wire:click="setActiveTab('ptst')" class="px-4 py-2 text-sm font-medium leading-5 focus:outline-none transition-colors duration-200 
            @if($activeTab === 'ptst') text-red-600 border-b-2 border-red-600 @else text-gray-500 hover:text-gray-700 @endif">
            Daftar Karyawan
        </button>
        <button wire:click="setActiveTab('non-ptst')" class="px-4 py-2 text-sm font-medium leading-5 focus:outline-none transition-colors duration-200 
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="py-3 px-4 text-sm font-semibold text-gray-600 text-left">Golongan Darah</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($items as $karyawan)
                        <tr>
                            <td class="py-3 px-4 text-sm text-gray-700">{{ ($items->currentPage()-1) * $items->perPage() + $loop->iteration }}</td>
                            ...
                            <td class="py-3 px-4 text-sm text-gray-700">{{ $karyawan->golongan_darah }}</td>
            ...
            <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="py-3 px-4 text-sm font-semibold text-gray-600 text-left">No HP</th>
                        <th class="py-3 px-4 text-sm font-semibold text-gray-600 text-left">Golongan Darah</th>
                        <th class="py-3 px-4 text-sm font-semibold text-gray-600 text-left">Perusahaan</th>
                        <th class="py-3 px-4 text-sm font-semibold text-gray-600 text-center">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    @forelse ($items as $pesertaMcu)
                        <tr>
                            ...
                            <td class="py-3 px-4 text-sm text-gray-700">{{ $pesertaMcu->no_hp }}</td>
                            <td class="py-3 px-4 text-sm text-gray-700">{{ $pesertaMcu->golongan_darah }}</td>
                            <td class="py-3 px-4 text-sm text-gray-700">{{ $pesertaMcu->perusahaan_asal }}</td>
                            {{-- Di dalam perulangan (@forelse ($items as $pesertaMcu)) --}}
                            <td class="py-3 px-4 text-sm text-gray-700 text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    
                                    {{-- Tombol Detail --}}
                                    {{-- Tombol detail ini harus mengarah ke detail PesertaMcu itu sendiri --}}
                                    <a href="{{ route('pasien.show', ['pesertaMcu' => $pesertaMcu->id]) }}" class="text-blue-600 hover:text-blue-900 transition-colors duration-200" title="Detail Pasien">
                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    </a>

                                    {{-- Tombol Edit --}}
                                    <a href="{{ route('keluarga.edit', ['keluarga' => $pesertaMcu->id]) }}" class="text-green-600 hover:text-green-900 transition-colors duration-200" title="Edit Data Pasien">
                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                    ...
                    @empty
                        <tr>
                            <td colspan="12" class="py-4 text-center text-gray-500">Data yang Anda cari tidak ditemukan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
