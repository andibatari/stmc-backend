            ...
            @foreach ($polis as $poli)
                @if ($activeTab === 'poli-' . $poli->id)
                    <div class="p-3 md:p-4 bg-gray-50 rounded-lg shadow-inner">
                        ...
                        {{-- RENDER KOMPONEN LIVEWIRE --}}
                        @if (strtoupper($poli->nama_poli) === 'GIGI')
                            @livewire('poli-gigi-form', [ 'jadwalId' => $jadwal->id ?? null, 'poliData' => $jadwalPoliData[$poli->id] ?? null ], key('gigi-'.$poli->id))
                        
                        @elseif (strtoupper($poli->nama_poli) === 'KEBUGARAN')
                            @livewire('kebugaran-form', [ 'patient' => $patient, 'jadwalPoliId' => $jadwalPoliData[$poli->id]->id ?? null, 'poliData' => $jadwalPoliData[$poli->id] ?? null ], key('kebugaran-'.$poli->id))

                        @elseif (strtoupper($poli->nama_poli) === 'FISIK')
                            @livewire('poli-fisik-form', [ 'patient' => $patient, 'jadwalId' => $jadwalPoliData[$poli->id]->id ?? null, 'poliData' => $jadwalPoliData[$poli->id] ?? null ], key('fisik-'.$poli->id))
